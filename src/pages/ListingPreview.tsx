...
const ListingPreview: React.FC = () => {
    ...
    const displayPrice = price || 1200000;
    const displayDownPayment = downPayment || 360000; // 30% of 1.2M
    const displayRate = interestRate || 6;
    const displayTerm = termYears || 30;
    const displayMonthly = monthlyPayment || 6000;
    ...
    return (
        <div style={{ minHeight: '100vh', backgroundColor: '#fff', paddingBottom: '80px' }}>
            ...
            <div style={{ padding: '16px 24px', borderBottom: '1px solid #e5e7eb', display: 'flex', justifyContent: 'space-between', position: 'sticky', top: 0, backgroundColor: 'white', zIndex: 50 }}>
                <button
                    onClick={() => navigate('/seller-financing-terms')}
                    style={{ background: 'none', border: 'none', display: 'flex', alignItems: 'center', gap: '8px', fontSize: '14px', fontWeight: 500, cursor: 'pointer' }}
                >
                    <ChevronLeft size={18} /> Back to edit terms
                </button>
                <div style={{ display: 'flex', gap: '12px' }}>
                    <Button variant="outline" style={{ fontSize: '13px', height: '36px', padding: '0 16px' }}>Save Draft</Button>
                    <Button onClick={() => navigate('/marketplace', {
                        state: {
                            photos: activePhotos, price, downPayment, interestRate, termYears, monthlyPayment, riskCategory, zestimate
                        }
                    })} style={{ backgroundColor: '#000', fontSize: '13px', height: '36px', padding: '0 24px' }}>Publish Listing</Button>
                </div>
            </div>
            ...
            <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '24px' }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 380px', gap: '48px' }}>
                    ...
                    <div style={{ position: 'sticky', top: '100px' }}>
                        ...
                        <Button style={{ width: '100%', marginTop: '16px', backgroundColor: 'black', borderRadius: '12px', height: '48px', fontSize: '15px' }}>
                            Request a visit
                        </Button>
                    </div>
